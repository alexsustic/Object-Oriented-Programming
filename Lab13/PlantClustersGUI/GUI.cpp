...
void GUI::initGUI()
{
	...
	buttonsLayout->addWidget(this->undoButton, 1, 2);
...
void GUI::myList()
{
	if (this->modeB == 1 && this->fileOpenAssistant == 1)
	{
		PlantClusterTableModel* model = new PlantClusterTableModel{ this->serv.getAllPlants() };
		PlantClustersWidget* mylist = new PlantClustersWidget{ model };
		mylist->show();
...
void GUI::undo()
{
	try {
		...
		QMessageBox::critical(this, "Error", "You can not perform any undo!");
...
void GUI::redo()
{
	try {
		...
		QMessageBox::critical(this, "Error", "You can not perform any redo!");
