#include "Service.h"
DynamicVector<PlantCluster> Service::getAllPlantClustersInRepo() const {
	return this->repository.getPlantCrusts();
}

DynamicVector<PlantCluster> Service::getAllPlants() const {
...
int Service::service_save(const std::string& codedName) {
	...
	DynamicVector<PlantCluster> plants = this->getAllPlantClustersInRepo();
	for (int index=0; index< plants.getSize(); index++)
		if (plants.getElement(index).getCodedName() == codedName)
		{
		    found_plant = plants.getElement(index);
			break;
		}
	return this->repoAssistant.savePlant(found_plant);
}

PlantCluster Service::service_next() {

	return repository.repo_next();
}
